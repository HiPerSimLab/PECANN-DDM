...
from losses import physics_loss, boundary_loss, avg_if_loss, u_exact
...
def training(para_adapt, rank, count, epochs, model, freq,
    ...
    for epoch in range(epochs):
        def _closure():
            model.eval()
            avg_pde_loss = physics_loss(model, x_dm,y_dm, freq)
            avg_bc_loss  = boundary_loss(model, x_bc,y_bc, freq)
...
def evaluate_write(model, domain, test_dis, rank, size, methodname, trial, write=False):
    ...
    u_star     = u_exact(x_test,y_test,freq)
...
outer_iter = 8000
epochs     = 100
freq      = torch.tensor([40., 2., 2., 2.], device = device)
...
test_dis = [640//dims[0] ,640//dims[1]]

n_h_layers = 3
n_neurons  = 20

para_adapt = ParaAdapt(zeta=0.99, omega=0.999, eta=torch.tensor([[1.],[0.01]]), epsilon=1e-16, epoch_min=50)
methodname = f'psn_simple{int(freq[0].cpu())}_{int(freq[2].cpu())}_xy{dims[0]}{dims[1]}_nn{n_h_layers}_{n_neurons}_dom_dis{mesh}'
...
for trial in range(1, trials+1):
    ...
    for count in range(1, outer_iter + 1):
        ...
        model, q, Lambda, Mu, mu_max, Bar_v, optimizer, optim_q = training(para_adapt, rank, count, epochs, model, freq,
                x_dm,y_dm, x_bc,y_bc, x_int,y_int,
                u_adj, dudn_adj, dudt_adj,
                q, Lambda, Mu, mu_max, Bar_v, optimizer, optim_q,
                outer_s, mu_s, lambda_s, constr_s, object_s, q_s, loss_s)
        ...
        l2, linf = evaluate_write(model, domain, test_dis, rank, size, methodname, trial)
        l2_s.append(l2)
        linf_s.append(linf)
